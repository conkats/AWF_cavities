...
use AWF_num        ! module for numerical integration of 1D momentum
use field_operator ! need for the gradient calls
use adapwf         ! module for initialising and tagging cells
use period         ! need for gradient sync
!===============================================================================
...
double precision,dimension(:),pointer::  cvar_utau
...
!===============================================================================
integer          ifac_n, j 
double precision :: wss, trap, dUdycofimp,  tempcbi
integer          :: inc, iprev, iccocg, irangv, ivart,ifcvsl
double precision,  allocatable, dimension(:,:) :: gradp, gradt
double precision,  allocatable, dimension(:,:,:) ::  gradv
! For convective terms and geometric parameters of the near wall node
double precision ::y_n, utau_n, utauI
double precision u_nx, u_ny, u_nz, rnx_n, rny_n, rnz_n, pnd
double precision u_sn, t_x, t_y, t_z, t_xn 
double precision yvisc, cpp, rkl
double precision alph, gdott, gdotn
double precision, dimension(:), allocatable :: advecMom, dPtdx_i
double precision, dimension(:), allocatable :: advecTemp, dTtdx_i
double precision, dimension(:), allocatable :: N_vel
character(80)   file_name
double precision u_nxii, u_nyii, u_nzii, u_nxjj, u_nyjj, u_nzjj
double precision uIt, vIt, wIt   
double precision temp_i, temp_n, Tn, qw_nawf, t_nawf
double precision, dimension (N) :: Tj_awf
double precision, dimension(:,:), pointer :: cvar_vel
double precision, dimension(:), pointer :: Temp_array, cvar_pres, viscls
double precision, dimension(:), pointer :: cvar_AWF
...
!============================AWF coding begins here============================
alph  = 2.295d-1
! Momentum arrays 
allocate(gradp(3,ncelet))
allocate(gradv(3,3,ncelet))
allocate(advecMom(nfabor))
allocate(dPtdx_i(nfabor))
allocate(N_vel(nfac))

! Thermal arrays 
allocate(advecTemp(nfabor))
allocate(dTtdx_i(nfabor))
allocate(gradt(3,ncelet))

! initialise the arrays
advecMom(1:nfabor)       = 0.d0
dPtdx_i(1:nfabor)        = 0.d0
dTtdx_i(1:nfabor)        = 0.d0
N_vel(1:nfac)            = 0.d0
gradp(1:3,1:ncelet)      = 0.d0
gradv(1:3,1:3,1:ncelet)  = 0.d0
advecTemp(1:nfabor)      = 0.d0
gradt(1:3,1:ncelet)      = 0.d0


inc = 1     ! 0: increment; 1: do not increment 
iprev = 0   ! 1 if values at previous time step should be used, 0 otherwise 
iccocg =  1 ! 1 or 0: recompute COCG or not 


!call the value for the pointers of velocity and then the gradient op
call field_get_val_v(ivarfl(iu), cvar_vel)
call field_get_val_s(ivarfl(ipr), cvar_pres)

! Physical properties and temperature :
! call the temperature field to get the near wall value at node I
! call the pointer to fill the temperature array for the neighbour interface Tn
if (iscalt.gt.0) then
  ivart = isca(iscalt)
  call field_get_val_s(ivarfl(ivart),Temp_array)
else
  write(nfecra,*) iscalt
  call csexit (1)
endif
...
! Synchronise the pointer value with all ranks before the gradients are computed
! For parallel calculations, if neighbours variables on interior faces are used, they must be synchronized
! The same case for periodicity
 if (irangp.ge.0.or.iperio.eq.1) then
  call synsca(Temp_array)
  call synsca(cvar_pres)
  call synvin(cvar_vel)
 endif

call field_gradient_scalar(ivarfl(isca(iscalt)), iprev, imrgra, inc, iccocg, gradt)

! call the pressure gradient and the velocity gradients
call field_gradient_scalar(ivarfl(ipr), iprev, imrgra, inc, iccocg, gradp)
...
do ifac = 1, nfabor
  ...
  if (icodcl(ifac,iu).eq.5) then
    ...
    !Define nodal temperature for AWF
    tempcbi= Temp_array(iel)

    !Define near wall cell height, y_N  
    y_n =  2*distb(ifac)

    ! Geometric quantities
    distbf = distb(ifac) !or else y_p
    srfbnf = surfbn(ifac)
    ...
    ! Unit normal vector 
    ! Transformation to a local coordinate system with tangential
    ! and normal vectors
    ! the normal unit vector points outwards by definition
    ...
    ! rcodcl is the boundary condition array/
    ! the value associated with that boundary condition
    ! with the respect to the velocity in node I
    ...
    ! CK: U_I' is calculated in condli.f90 
    ! see theory guide figure IV.D.1 pg 189
    ! the transformation refers
    ! needed for moving walls
    ! velipb is the velocity is x direction of node I with face index of the boundary

    upx = velipb(ifac,1) - rcodcx
    upy = velipb(ifac,2) - rcodcy
    upz = velipb(ifac,3) - rcodcz
    !usn = U_x cdot nx + U_y cdot ny +U_z cdot nz
    usn = upx*rnx+upy*rny+upz*rnz  ! velocity vector component normal to the wall
    tx  = upx -usn*rnx             ! velocity x component in a tangential to the wall
    ty  = upy -usn*rny             !          y  
    tz  = upz -usn*rnz             !          z 
    txn = sqrt(tx**2 +ty**2 +tz**2)! velocity mag vector tangent to the wall
    utau= txn                      ! velocity mag in a direction tangential to the wall
    ...
    if (itytur.eq.3) then
      ...
      ! CK: the transform matrix double underline P is calculated and stored in the array ELOGLO
      ! to convert the reference frame from local to global
    ...
    ! The transformation of the advection(convection) terms into local co-ordinate frame
  ...
  ( iwallf, ifac  ,                                        &
    ...
    !===============================AWF bits======================================
    ! utauI= U_I*t_i:
    ! tangential components of nodal velocity on node I
    ! utauI = U_I the cell centre velocity of the near wall cell
    utauI = upx*tx + upy*ty + upz * tz
      
    advecMom(ifac) = romc* ( utauI)  *(&
      tx * ( tx * gradv(1,1,iel)  + ty * gradv(1,2,iel) + tz * gradv(1,3,iel) )  + &
      ty * ( tx * gradv(2,1,iel)  + ty * gradv(2,2,iel) + tz * gradv(2,3,iel) ) + &
      tz * ( tx * gradv(3,1,iel)  + ty * gradv(3,2,iel) + tz * gradv(3,3,iel) ) )
      
    dPtdx_i(ifac) = ( tx * gradp(1,iel) ) + &
                      ( ty * gradp(2,iel) ) + &
                      ( tz * gradp(3,iel) )
    !convective terms in temperature equation
    advecTemp(ifac) =  romc*(utauI)* (& 
                       ( tx * gradt(1,iel)) + &
                       ( ty * gradt(2,iel)) + &
                       ( tz * gradt(3,iel)))

    !tangential main grid temperature gradient parallel to the wall for Gk
    dTtdx_i(ifac) = ( tx * gradt(1,iel)) + &
                      ( ty * gradt(2,iel)) + &
                      ( tz * gradt(3,iel))



    ! we need the face index of the north 
    ! facngh = face of the neighbour cell   
    ! index number of the two neighboring cells for each internal face
    ! ifacel calculation of internal face value 
    ! from the values of the variable in the neighbouring cells
    ifac_n = facngh(ifac,2) !index of the north neighbour 

    ii = ifacel(1, ifac_n)
    jj = ifacel(2, ifac_n)
    pnd = pond(ifac_n)

    ! if the index of the boundary cell is equal to 
    ! ii then jj is the north neighbour
    if (iel.eq.ii) then
      temp_i = Temp_array(ii)
      temp_n = Temp_array(jj)
    else
      temp_i = Temp_array(jj)
      temp_n = Temp_array(ii)
    endif 
    Tn = pnd*temp_i  + (1.d0 - pnd)*temp_n
    

    !select the iel =ifabor(ifac)
    ! if the index of the boundary cell is equal to 
    ! ii then jj is the norht neighbour
    if (iel.eq.ii) then
      u_nx = cvar_vel(1, jj) 
      u_ny = cvar_vel(2, jj) 
      u_nz = cvar_vel(3, jj) 
    ...
    ! compute the north velocity direction
    ! unit normal for North Neighbour Vel
    rnx_n = surfbo(1,ifac_n)/srfbnf
    rny_n = surfbo(2,ifac_n)/srfbnf
    rnz_n = surfbo(3,ifac_n)/srfbnf
    u_sn = u_nx*rnx + u_ny*rny + u_nz*rnz
    t_x = u_nx - u_sn*rnx
    t_y = u_ny - u_sn*rny
    t_z = u_nz - u_sn*rnz

    ! magnitude of the velocity of the north neighbour N
    t_xn = sqrt(t_x**2 + t_y**2 + t_z**2) 
    !can re-compute the tangential unit vectors for the North neighbour velocity
   

    !unit tangents for North Neighbour Vel
    if (t_xn.ge.epzero) then
      t_x = t_x/t_xn
      t_y = t_y/t_xn
      t_z = t_z/t_xn
    else
    ...
    ! gives the component vector in the tangential direction for N node
    N_vel(ifac_n) = u_nx*t_x + u_ny*t_y + u_nz*t_z
    
    ! node velocity value contribution from pond function
    ! is compared either closer to U_p or U_n
    utau_n = pnd*utauI + (1.d0 - pnd)*N_vel(ifac_n)
    ! CK: cofimp= coefficient B  which feeds to coefb in order to complete the matrix of
    ! that
    ! CK: hflui  = \mu/y * y^+/U^+  
    ! tau_w = rho*uet*uk begins here!

    ! utau refers to the U_I velocity of the node
    ! CK: call the module routine for the numerical integration 
    
    ! the tangential vectors will sort the orientation and
    ! the sign of the gravity vector
    ! i.e if wall is horizontal then t = (1,0,0) and g will be zero
    ! if the wall is vertical then t = (0,1,0) and g will be -9.81
    ! if Boussinesq and g is set to zero in gui
    ! the sign of g does matter
    !gdott = gx * tx + (gy) * ty + gz * tz
    !gdotn = gx * rnx + (gy) * rny + gz * rnz
    if (gy.ne.0.d0) then
      gdott = gx * tx + (gy) * ty + gz * tz
      gdotn = gx * rnx + (gy) * rny + gz * rnz
    else !for Ra=1.58e9
      gdott = gx * tx + (-9.81) * ty + gz * tz
      gdotn = gx * rnx + (-9.81) * rny + gz * rnz
    endif
    ! check if it is  a boundary cell with another boundary cell next to it
    ! if corner set uDTdxt to zero

    !  if (mrkrcel(ii).ge.1.and.mrkrcel(jj).ge.1) then
    !  !check if the cell index of the boundary cell is ii or jj from the 
      !norhtneighbour array
    !  if ( (ii.eq.iel) .or. (jj.eq.iel) ) then
    !    advecMom(ifac)  = 0.d0
    !    dPtdx_i(ifac)   = 0.d0
    !    advecTemp(ifac) = 0.d0
    !    dTtdx_i(ifac)   = 0.d0
    ...
    !call thermal before momentum wall function
    call oneD_grid_Thermal_diffus( ifac,icodcl(ifac,ivart), rcodcl(ifac,ivart,1), rcodcl(ifac,ivart,3),      &
    y_n, distbf, Tn, advecTemp(ifac),                                                                        &
    ek, ro0, cpp, rkl, visclc,                                                                               &
    qw_nawf, t_nawf, Tj_awf)
    
    call oneD_grid_momentum_diffus(ifac, distbf, advecMom(ifac), dPtdx_i(ifac), utau_n, y_n, &
                                   ro0, ek, visclc, dTtdx_i(ifac), gdott, gdotn, Tj_awf, wss,  trap, dUdycofimp)
    ...
    !-----------------------------
    rcodcn = rcodcx*rnx+rcodcy*rny+rcodcz*rnz
    ! setting all the coefficients to zero is equivalent to have symmetry condition
    ! U_F = to a value for the gradient computation

    !cofimp=replace Pk from the AWF 
    !source terms
    !to test: but in principle it should give the same as when replacing Pk
    !if (ntcabs.gt.1) then
    ! cofimp = 1 - ((distbf*romc) / (utauI*visclc) )*(2*sqrt (((visclc/visctc)) * trap) -dUdycofimp)
    ! !cofimp = 1 + (distbf)*(dUdycofimp-2*sqrt (((trap/visctc)) ))
    !endif
    ...
    ! U_F = \underline{A_{U_F}}+\underline{\underline{B}}U_I
    !
    !     IDENTITY =|1   0  0|
    !               |0   1  0|
    !               |0   0  1|according to the size of the other matrix
    ! n is a vector =|nx|
    !                |ny|
    !                |nz|
    ! point outwards from the surface i.e nx =-1
    ! n x n = the tensor product of two vectors gives a tensor

    ! IDENTITY- n x n = |1-nx^2 -nxny  -nxnz | 
    !                   |-nxny  1-ny^2 -nynz |
    !                   |-nxnz  -nynz  1-nz^2|
    !
    ! U_I    = |U_I,x|
    ...
    !
    !
    ! coefbu = \underline{\underline{B}}
    ! cofimp = U_F /U_I = 0 in the viscous sublayer and =1-3/2kU^+
    ! UF is a function of the tangential velocity of node I (U_I,x)
    ! [IDENTITY- n x n] \cdot  U_I 
    ! =
    !   |U_I,x-U_I,x nx^2       -nxnyU_I,y           -nxnzU_I,z        |
    !   |-U_I,x nynx            +U_I,x-U_I,xnyny     -U_I,znynz        |
    !   |-U_I,x nznx            -U_I,y nzny          -U_I,z-U_I,znznz  |
    !
    ! hflui  = \mu/y * y^+/U^+ = \mu*y*rho*uet*uk/y*\mu*U_I = rho*uet*uk /U_I
    ! hint = \mu/y or when turbulence hint = \mu+\mu_t/y

    ! \tau \cdot n = hlfui * [IDENTITY- n x n] \cdot  U_I 
    !  since we are solving for velocities in the momentum equation the shear stress for Newtonian
    ! fluids is Newtonian: \sigma_{ij} = \mu \frac{( \partial{U_i})}{\partial{x_j}} + \frac{( \partial{U_j})}{\partial{x_i}}
    ...
    ! Gradient
    !coefau(1,ifac)   = 0.d0! (1.d0-cofimp)*(rcodcx - rcodcn*rnx) + rcodcn*rnx
    !coefau(2,ifac)   = 0.d0! (1.d0-cofimp)*(rcodcy - rcodcn*rny) + rcodcn*rny
    !coefau(3,ifac)   = 0.d0! (1.d0-cofimp)*(rcodcz - rcodcn*rnz) + rcodcn*rnz
    !coefbu(1,1,ifac) = 0.d0! cofimp*(1.d0-rnx**2)
    !coefbu(2,2,ifac) = 0.d0! cofimp*(1.d0-rny**2)
    !coefbu(3,3,ifac) = 0.d0! cofimp*(1.d0-rnz**2)
    !coefbu(1,2,ifac) = 0.d0! -cofimp*rnx*rny
    !coefbu(1,3,ifac) = 0.d0! -cofimp*rnx*rnz
    !coefbu(2,1,ifac) = 0.d0! -cofimp*rny*rnx
    !coefbu(2,3,ifac) = 0.d0! -cofimp*rny*rnz
    !coefbu(3,1,ifac) = 0.d0! -cofimp*rnz*rnx
    !coefbu(3,2,ifac) = 0.d0! -cofimp*rnz*rny

    ! Flux boundary conditions define for AWF
    ! in condli is used to compute the shear and then 
    ! multiply by the surface area of boundary face
    ! wss.t=(wss*tx + wss*ty + wss * tz)
    cofafu(1,ifac)   = wss*tx!-hflui*(rcodcx - rcodcn*rnx) - hint*rcodcn*rnx
    cofafu(2,ifac)   = wss*ty!-hflui*(rcodcy - rcodcn*rny) - hint*rcodcn*rny
    cofafu(3,ifac)   = wss*tz!-hflui*(rcodcz - rcodcn*rnz) - hint*rcodcn*rnz
    cofbfu(1,1,ifac) = 0.d0!!hflui*(1.d0-rnx**2) + hint*rnx**2
    cofbfu(2,2,ifac) = 0.d0!!hflui*(1.d0-rny**2) + hint*rny**2
    cofbfu(3,3,ifac) = 0.d0!!hflui*(1.d0-rnz**2) + hint*rnz**2
    cofbfu(1,2,ifac) = 0.d0!(hint - hflui)*rnx*rny
    cofbfu(1,3,ifac) = 0.d0!(hint - hflui)*rnx*rnz
    cofbfu(2,1,ifac) = 0.d0!(hint - hflui)*rny*rnx
    cofbfu(2,3,ifac) = 0.d0!(hint - hflui)*rny*rnz
    cofbfu(3,1,ifac) = 0.d0!(hint - hflui)*rnz*rnx
    cofbfu(3,2,ifac) = 0.d0!(hint - hflui)*rnz*rny
    ...
    if (itytur.eq.2) then
      ...
      !----------------------------------
      !pimp set the Dirichlet value at the node
      if (iuntur.eq.1) then
        pimp = uk**2/sqrcmu
      else
        pimp = 0.d0
     ...
     call set_dirichlet_scalar &
      ...
      !     (xkappa*efvisc**2*(yplus+dplus)**2)! =4*(uk**3)/(xkappa*distbf)
      ! else
      !  qimp = 0.d0
      !end if
      !Dirichlet on epsilon
     !set this if ikecou =0 and bigg source term
     !works:
     !or impose the value at the near wall node
      if (yplus.ge.5.1d0) then
        pimp = (cvar_k(iel)**3/2)/(2.55d0*distbf)!=uk**3/xkappa*distbf
      else
        pimp=2.0d0*(visclc/romc)*cvar_k(iel)/5.1d0*2!=0.d0
      endif
      call set_dirichlet_scalar &
         !====================
...
use AWF_num       !AWF module  from numerical integration of T
use field_operator
use adapwf        !module for initialising/tagging cells
use period
...
!AWF vars
!==============================================================================
integer          inc, iprev, iccocg,ifac_n, ii,jj
double precision tx, ty , tz , txn, uIt, vIt, wIt, utauI
double precision  rnx, rny,rnz, rxnn, pnd, y_north
double precision srfbnf, upx, upy, upz , usn, txn0
double precision qw_nawf, t_nawf, ek 
double precision TempSource, temp_i, temp_n, Tn
double precision, dimension(N) :: Tj_awf
double precision, dimension(:,:), pointer :: velI
double precision,  allocatable, dimension(:,:) :: gradt, velipb
double precision, dimension(:), allocatable:: advecTemp
double precision, dimension(:), pointer :: cvar_k, cvar_r11, cvar_r22, cvar_r33
double precision, dimension(:), pointer :: Temp_array
double precision, dimension(:), pointer :: cvar_AWF2
...
if (b_f_id .ge. 0) then
  ...
  ! itempb refers to the wall temperature index
...
!===================Thermal =AWF===============================================
! AWF variables/arrays begins here
allocate(advecTemp(nfabor))
allocate(gradt(3,ncelet))
...
!call the pointer to fill the temperature array for Tn
call field_get_val_s(ivarfl(isca(iscalt)), Temp_array)
! call the pressure gradient and the velocity gradients
! call the nodal velocity in x-direction for the convective terms
! and utauI (U.t)
call field_get_val_v(ivarfl(iu), velI)


!synchronise the pointer value with all ranks before the gradients computation
if (irangp.ge.0.or.iperio.eq.1) then
  call synsca(Temp_array)
  call synvin(velI)
endif

!see condli.f90 line 661 on how to compute the velocity in i 
do isou = 1, 3

  do ifac = 1, nfabor
    iel = ifabor(ifac)
    velipb(ifac,isou) = velI(isou,iel)
  enddo
...
if (itytur.eq.2 .or. iturb.eq.50    &
     .or. iturb.eq.60.or. iturb.eq.21) then
  call field_get_val_s(ivarfl(ik), cvar_k)
else if (itytur.eq.3) then  
  call field_get_val_s(ivarfl(ir11), cvar_r11)
  call field_get_val_s(ivarfl(ir22), cvar_r22)
...
do ifac = 1, nfabor
  ...
  if (icodcl(ifac,iu).eq.5) then
    ...
    !======================================AWF==========================================
    ! Near wall cell height, y_N 
    y_north =  2*distb(ifac)
    srfbnf = surfbn(ifac)!the norm of the surface of the boudary faces
    !===========================================================================
    ! 1. Local framework (from clptur_computing the U.t)
    !===========================================================================
    
    !Unit normal vector 
    !Transformation to a local coordinate system with tangential and normal vectors
    !the normal unit vector points outwards by definition
    rnx = surfbo(1,ifac)/srfbnf!surfbo = surface vector of the boundary faces. 
    rny = surfbo(2,ifac)/srfbnf
    rnz = surfbo(3,ifac)/srfbnf

    ! Handle displacement velocity

    ! rcodcl is the boundary condition array/the value associated with that boundary condition
    ! with the respect to the velocity in node I

    ! Relative tangential velocity
    upx = velipb(ifac,1) !vel(iel,1) 
    upy = velipb(ifac,2) !vel(iel,2) 
    upz = velipb(ifac,3) !vel(iel,3) 
    usn = upx*rnx+upy*rny+upz*rnz!usn = U_x cdot nx + U_y cdot ny +U_z cdot nz
    tx  = upx -usn*rnx!   the velocity x component in a direction  tangential to the wall
    ty  = upy -usn*rny!                y 
    tz  = upz -usn*rnz!                z 
    txn = sqrt(tx**2 +ty**2 +tz**2)! magnitude of the velocity vector tangent to the wall

    ! Unit tangent

    if (txn.ge.epzero) then
      ! finds the unit vector by doing a division of the tangential over its velocity vector magnitude 
  
        txn0 = 1.d0
        !REF. Ferziger and Peric page 257 eq. 8.76
        tx  = tx/txn! finds the unit vector by doing a division of the tangential overwrites velocity vector magnitude 
        ty  = ty/txn
        tz  = tz/txn
    else
    ...
    elseif (itytur.eq.3) then
     ...
     !convective terms for thermal field
      advecTemp(ifac) =  romc*( utauI) * (&
                         ( tx * gradt(1,iel)) + &
                         ( ty * gradt(2,iel)) + &
                         ( tz * gradt(3,iel)))
    ...
    ! facngh = face of the neighbour cell  
    ! has to be 2 because we need the face index of the north 
    ! neighbour 1 gives the face at the wall  
    ! index number of the two neighboring cells for each internal face
    ! ifacel calculation of internal face value 
    ! from the values of the variable in the neighbouring cells
    ! get the interface between the two cells 
    ! check for which is the index of the north neighbour
    ifac_n = facngh(ifac,2) !index of the north neighbour
    ii = ifacel(1, ifac_n)
    jj = ifacel(2, ifac_n)
    pnd = pond(ifac_n)
    ...
    Tn = pnd*temp_i  + (1.d0 - pnd)*temp_n
      !switch on to set zero for UdT/dx_t in corners
      !if (mrkrcel(ii).ge.1.and.mrkrcel(jj).ge.1) then
      !check if the cell index of the boundary cell is ii or jj from the 
      !norhtneighbour array
      !  if ( (ii.eq.iel) .or. (jj.eq.iel) ) then
    ...
    ! Default condition in the code is Neumman
    ! ensure that hint is computed in the laminar region
    ! hint = rkl/distbf
    if (iturb.ne.0.and.icodcl(ifac,ivar).eq.5) then
     call hturbp(iwalfs,prdtl,turb_schmidt,yplus,dplus,hflui,ypth)
     ! Compute (y+-d/scalar+)/T+ *PrT
     yptp = hflui/prdtl
    ...
    elseif (iturb.ne.0.and.icodcl(ifac,ivar).eq.3) then
     !for zero heat flux leave the wall function of the code
     call hturbp(iwalfs,prdtl,turb_schmidt,yplus,dplus,hflui,ypth)
     hbord2(ifac) = rkl/distbf * hflui
     !!!!!! y+/T+
     yptp = hflui/prdtl
     hflui = hint! = ((k+cp*mu_t)/sigma_t)/y
     !*htur = cp.mu/k . y+/T+ = mu.cp/k  . y.q/(T_I-T_wall).mu.cp 
    else
      ! y+/T+ *PrT
      yptp = 1.d0/prdtl
      hflui = hint
    ...
    ! Note that  ityturt(iscal) == 0 is the SGDH
    if (icodcl(ifac,ivar).eq.5) then
      ...
      if (ityturt(iscal).ge.1) then
        ...
        ! enters here in Betts cavity
        cofimp = 1.d0 - heq/hint
      endif

      ! To be coherent with a wall function, clip it to 0
      cofimp = max(cofimp, 0.d0)
      ! change coefficients A and B for the flux at the face

       !CK: modifications to coefficients instead of hflui  
      !if tw imposed=>qwall computed and vice versa
      ! For Dirichlet:
      ! Test of setting the B coefficient to zero and 
      ! setting the the flux in the A only
      ! modifications to coefficients instead of hflui
      ! if tw imposed=>qwall computed and vice versa
      ! modifications to coefficients instead of hflui
      !=======================AWF=================================================
      call oneD_grid_Thermal_diffus(ifac,icodcl(ifac,ivar),rcodcl(ifac,ivar,1), &
      rcodcl(ifac,ivar,3), y_north,              &
      distbf, Tn, advecTemp(ifac),               &
      ek, ro0, cpp,rkl, visclc,                  &
      qw_nawf, t_nawf, Tj_awf)
      coefap(ifac) = t_nawf!wall temperature
      coefbp(ifac) = 0.d0! heq 
      
      cofafp(ifac) = qw_nawf!qflux_global(ifac)!wall flux
      cofbfp(ifac) = 0.d0
      ...
      if (ityturt(iscal).eq.3) then
        ...
        !CK: ypth value of the limit for \f$ y^+ 
      ...
      !               le flux est le bon et le coef d'echange aussi)yptp
      !      De meme dans condli.yptp
    ...
    if (b_f_id.ge.0 .or. iscal.eq.iscalt) then
      ...
      ! AWF
      ! flux boundary condition from A+B.T_I
      ! setting the the flux in the A only
      ! For Dirichlet
      if (icodcl(ifac,ivar).eq.5) then
        if (iscal.eq.iscalt) then     
          phit = cofafp(ifac)+cofbfp(ifac)*theipb(ifac)
          ! change only the flux for the dirichlet
          ! from my wall function
          !phit = qw_nawf
        else
        ...
        elseif (icodcl(ifac,ivar).eq.3) then   
          ...
          coefap(ifac) = t_nawf!Twall_global(ifac)!
          coefbp(ifac) = 0.d0
          cofafp(ifac) = qw_nawf!rcodcl(ifac,ivar,3)!!qflux_global(ifac)!
          cofbfp(ifac) = 0.d0
          phit = qw_nawf
          !====================AWF===============================================
          ! CK: in Bett's cavity it does not enter this
          ...
          !in case adiabatic conditions are prescribed in smooth walls
          !if (yptp <0.d0) then
          !  tplus = 0.d0
          !else
          !  tplus = max((yplus-dplus), epzero)/yptp!CK: yptp cannot be zero!!!!!!!!!!!
          !endif
          
          !  do not worry about those since it is just for post processing of the Nusselt
          ! number which I can get from the heat flux of my wall function solution
          ! they way code_saturne works is by getting the Nu which relies on the log-law
          tet = phit/(romc*cpp*max(yplus-dplus, epzero)*xnuii/distbf)
          ! T+ = (T_I - T_w) / Tet
          ! tplus = (val_s(iel)- t_wnumer)/max(tet,epzero)
          ! T+=y+/y+/T+=T_I-T_wall.rho.cp.uk/qw
          tplus = max((yplus-dplus), epzero)/yptp!CK:University.

          ! if (b_f_id .ge. 0) bval_s(ifac) =  bval_s(ifac) - tplus*tet! 
          !==========================AWF=============================================
          if (b_f_id .ge. 0) bval_s(ifac) = t_nawf!Twall_global(ifac)!
       
          !if (icodcl(ifac,ivar).eq.5) then
          !  if (b_f_id .ge. 0) bval_s(ifac) = t_nawf
          !else
          !  if (b_f_id .ge. 0) bval_s(ifac) =  bval_s(ifac) - tplus*tet
          ! endif  
          !==================AWF========================================================
