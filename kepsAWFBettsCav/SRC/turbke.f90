...
! Local vars AWF 
integer                            :: ilelt, nlelt, facecount, ii
integer, allocatable, dimension(:) :: lfbrps
character(100)                     :: file_name
double precision                   :: beta, big,  y_visc
double precision                   :: yd, ydstar, ystar, ynstar, y_n, cl
double precision, allocatable, dimension(:,:) ::  gradt
double precision, dimension(:), pointer       ::  cvar_scalt
double precision, allocatable, dimension(:)   :: epsimp
double precision, allocatable, dimension(:)   :: Ksinksource, eps_avg, epsv_avg
!YAP term variables
integer          icodcl(nfabor,nvar), infpar
double precision rcodcl(nfabor,nvar,3)
double precision distxn, dispa(ncelet)
double precision,dimension(:),allocatable::lscale, SEYAP
...
!-----------------------------------
! YAP term arrays
allocate(lscale(ncelet))
allocate(SEYAP(ncelet)) 
!-----------------------------------
...
!     Here is computed  G =-g.grad(rho)/prdtur/rho
 ...
 ( f_id0  , imrgra , inc    , iccocg , nswrgp , imligp ,          &
  ...
  !===============================================================================
  ! call for the temperature gradient for the Gk term modified here
  ! dT_i/dx_j (ixiel vector)
  if (iscalt.gt.0) then
  ...
  beta = 1/(t0+273.15) ! thermal expansion coefficient
  ...
  do iel = 1, ncel
    ...
    xk   = cvara_k(iel)
    ttke = xk / xeps
    ! w2 = g/rho*Prt * drhodx = g/rho*Prt*rho*beta*g*dTdx
    ! goes both to k and epsilon
    if (gy.ne.0.d0) then
       gravke =(beta *( gradt(1,iel)*gx + gradt(2,iel)*(gy) + gradt(3,iel)*gz ) / &
               (prdtur))![kg.m/s^3]
    else
      gravke = (beta *( gradt(1,iel)*gx + gradt(2,iel)*(-9.81) + gradt(3,iel)*gz ) / &
                (prdtur))![kg.m/s^3]
    endif
...
if (itytur.eq.2) then
  ...
  do iel = 1, ncel
    ...
    rho   = cromo(iel)
    !smbrk=Pk-rho.eps-2/3 rho.k div.u
...
call cs_user_turbulence_source_terms &
 ( nvar   , nscal  , ncepdp , ncesmp ,                            &
  ...
  !==============================================================================
  ! YAP term correction to epsilon
  !  The yap term source term applies to all the cell centres
  !  and requires the distance of each node with the wall
  !  similar algorithms are used in  1 equation models and in the LRR wall echo
  !  and also in the k-w turbkw.f90 routine as as in the routine
  !  note that the yap term outside the viscous layer is  small
  !  for wall distance see dispar(iel) and the routine  distpr.f90, tridim.f90
  !===============================================================================
  ...
  !   dt     ,                                                       &
  !   rcodcl )
  !enabling the YAP term for a HIgh-Re improves the velocity and k predictions 
  if (.true.) then
    if (itytur.eq.2 .and.iturb.eq.20) then
       call user_boundary_conditions(nvar, itypfb, icodcl, rcodcl)
       ! Number of wall faces for betts cavity should be 400 for a Low Re
       infpar = 0
       do ifac = 1, nfabor
       ...
       if (infpar.eq.0) then
         ...
         !care on this call function
         !important to check again the way the dispa is called/computed/give the right value!!!!
         !if (abs(icdpar).eq.1) then
           call distpr(itypfb, dispa)
         !else if (abs(icdpar).eq.2) then
          ! call distpr2(itypfb, dispa)
         !endif
       endif
       if (irangp.ge.0.or.iperio.eq.1) then
        call synsca(dispa)
       ...
       !  print*, distxn, dispa(iel) , xyzcen(1,iel),xyzcen(2,iel),xyzcen(3,iel)
         lscale(iel) =( (cvar_k(iel)**1.5d0 )/cvar_ep(iel) ) / (2.55* MAX(distxn,1.d-12))
         SEYAP(iel) = max ( (0.83* ( (cvar_ep(iel)**2)/cvar_k(iel))* (  lscale(iel) -1) &
          *lscale(iel)**2 ), 0.d0) 
         w8(iel) = cell_f_vol(iel)*SEYAP(iel)
       enddo
   endif
 endif
!==============================================================================

!AWF source terms on turbulence equations (for Pk and epsilon in the turb. model) 
allocate(lfbrps(nfabor))
allocate(Ksinksource(ncelet))
allocate(eps_avg(nfabor))
allocate(epsv_avg(nfabor))
allocate(epsimp(nfabor))

call getfbr('hot or cold or bottom or top', nlelt, lfbrps)

! call field_get_val_s(iviscl, c_viscl) 

cl     = 2.55d0
ydstar = 5.1d0
big    = rinfin! 1d.+30!if it is not too big it does not work

! Do an average in case Pk(ifac) is at a corner
! and it has 2 or 3 contributions
! Take the average
! and fill an array for Pk/production computed from 
! each face (array for each cell) from AWF
! check for area weighted average
...
if (itytur.eq.2 .and.iturb.eq.20) then
   ! rhoDKDt[kg/m3 . m2/s3]=Pk-Gk-rho*epsilon + mu*div(rho*u)
   ! replacing Pk, epsilon from the code with the  Pkave,eps_avg
   Do ilelt = 1, nlelt
      ! connectivity between cell centre and near wall face id
      ifac   = lfbrps(ilelt)    ! ifac:id of the boundary face
      iel    = ifabor(ifac)     ! associated boundary cell index
      y_n    =  2*distb(ifac)   ! nw cell height 
      yd     = ((cpro_pcvlo(iel)/cromo(iel))*2*cl)/ (cvar_k(iel)**0.5d0)
      ystar  = (sqrt(cvar_k(iel))*distb(ifac))/(cpro_pcvlo(iel)/cromo(iel))
      ynstar = (sqrt(cvar_k(iel))*y_n)/(cpro_pcvlo(iel)/cromo(iel))
      !Ksinksource = cell.vol(Pk+Gk-rho.epsilon)
      Ksinksource(iel) = cell_f_vol(iel)*(cpro_pcvto(iel)*strain(iel) -cromo(iel)* cvar_ep(iel)-                &
                           d2s3*cromo(iel)*cvar_k(iel)*divu(iel)  +                                             &      
                          cpro_pcvto(iel)*(-beta *( gradt(1,iel)*gx + gradt(2,iel)*(-9.81) + gradt(3,iel)*gz ) / &
                           (prdtur)) )
         if (ynstar .gt. ydstar) then! or if (y_n .GT. yd) then
        ! Expressions is derived from Craft's 2002 paper eq.3.12 
        eps_avg (ifac) = ( ((cvar_k(iel) **1.5d0)/ y_n ) ) * ( (1.0 / cl)   &
        + log(y_n/yd)/cl ) 
         w7(iel) = ( cell_f_vol(iel)*cromo(iel)* (global_trap_Pk(ifac)+ GKaveg_global(ifac) -  eps_avg(ifac)))&
        - Ksinksource(iel)              
      else!if (ynstar.LT. ydstar) then
          ! in the viscous sublayer
          epsv_avg(ifac)= (2.d0*(cpro_pcvlo(iel)/cromo(iel))*cvar_k(iel))/(yd**2)
           w7(iel) = ( cell_f_vol(iel)*cromo(iel)* (global_trap_Pk(ifac)+ GKaveg_global(ifac) -  epsv_avg(ifac)))&
          - Ksinksource(iel)

      endif
      ! epsilon condition depends on the near wall node location
      ! Gerasimov thesis equation 3.37 pg44
      ! Versteeg page 278 Versteeg  
      ! ikecou = 0 in cs_user_parameters in order for the -ve implicit part
      ! or based on Henkes paper 1995
      ! usimpe(iel) !(should be -ve so it is taken to L.H.S) 
      ! switch on when yplus value is greater than 20 
      ! number should be bigger than the rest of the balances in the equation of epsilon
      ! switch on when yplus value is greater than 20
      if (.false.) then  
          if (ystar.gt.ydstar) then! should be ystar but in stream is ynstar  
            epsimp(iel)  =   (cvar_k(iel)**1.5d0)  / (cl* MAX(distb(ifac),1.d-12))!
          else
            epsimp(iel)  =   (2*(cpro_pcvlo(iel)/cromo(iel))*cvar_k(iel) )/( yd**2)!
          endif

          w8(iel)     = big *cell_f_vol(iel) * epsimp(iel)!
          usimpe(iel) = -big* cell_f_vol(iel) !(should be -ve)
        endif
...
! it enters here, the case for my sources
else
  do iel = 1, ncel
    ! Explicit part
...
!                                     smbrk, smbre
!===============================================================================
! source term to turbulence when adding mass to the system
if (ncesmp.gt.0) then
